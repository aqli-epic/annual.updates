...
# Global GBD
trendlines_aqli_data_global <- gadm2_aqli_2021 %>%
...
ar_global_fig1.2_data <- gadm2_aqli_2021 %>%
...
ar_global_fig1.2 <- ar_global_fig1.2_data %>%
  ggplot() +  geom_line(mapping = ggplot2::aes(x = years, y = pop_weighted_avg_pm2.5, color = interaction(region), linetype = interaction(region), lwd = 0.7), lwd = 2) +
  ...
  scale_color_manual(values = c("South Asia" = "darkred",
    "China" = "orange", "Rest of the World" = "khaki", "Global" = "snow4"), 
    breaks = c("South Asia", "China", "Rest of the World", "Global"), name = "legend") +
  scale_linetype_manual(values = c("South Asia" = "dashed",
    "China" = "dashed", "Rest of the World" = "dashed", "Global" = "solid"), name = "legend") +
