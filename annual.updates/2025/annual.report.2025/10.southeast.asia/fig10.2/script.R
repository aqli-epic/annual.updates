...
ar_se_asia_fig10.2_data <- gbd_results_master_2025 %>%
  filter(cause_of_death %in% diseases_list, country %in% country_list)

# making the 'location' column of type factor
ar_se_asia_fig10.2_data$country <- factor(ar_se_asia_fig10.2_data$country,
                                         levels = country_list)

# Converting 'cause_of_death' to type factor
ar_se_asia_fig10.2_data$cause_of_death <- factor(ar_se_asia_fig10.2_data$cause_of_death, levels = diseases_list)

# wrapping x-axis labels text
levels(ar_se_asia_fig10.2_data$cause_of_death) <- str_wrap(levels(ar_se_asia_fig10.2_data$cause_of_death), 30)

# reorder within each location as per the total life years lost column
ar_se_asia_fig10.2_data <- ar_se_asia_fig10.2_data %>%
...
# southeast asia figure 10.2: GBD
ar_se_asia_fig10.2 <- ar_se_asia_fig10.2_data %>%
