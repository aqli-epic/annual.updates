...
gbd_results_thailand <- gbd_results_master_2025 %>%
  filter(cause_of_death %in% c("Ambient ozone pollution","Child and maternal malnutrition","Childhood sexual abuse and bullying",
                ...
                "Tobacco","Transport injuries","Non-optimal temperature","Other environmental risks","Unintentional injuries","Unsafe sex","Unsafe water, sanitation, and handwashing"),country == "Thailand")

# filtering out those causes of death that are sort of* covered under PM2.5 in some broad way
colnames(gbd_results_thailand)[3] <- c("llpp_who_2023")
...
thailand_fs_fig2 <- thailand_fs_fig2_dataset %>%
  ...
  scale_y_continuous(breaks = seq(0, 6, 0.5), limits = c(0, 2)) +
