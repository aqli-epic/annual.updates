...
indonesia_fs_fig1_dataset <- gadm2_aqli_2023 %>%
...
indonesia_fs_fig1 <- indonesia_fs_fig1_dataset %>%
  ...
  geom_sf(data = gadm1_aqli_2023_shp %>% filter(name0 == "Indonesia"), color = "azure4", fill = "transparent", lwd = 0.15) +
  geom_sf(data = gadm0_aqli_2023_shp %>% filter(name0 == "Indonesia"), color = "cornsilk4", fill = "transparent", lwd = 0.5) +
  ggthemes::theme_map() +
