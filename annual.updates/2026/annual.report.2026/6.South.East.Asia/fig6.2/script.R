...
ar_se_asia_fig_data <- gbd_results_master_2026 %>%
  filter(cause_of_death %in% diseases_list, country %in% country_list)

# making the 'location' column of type factor
ar_se_asia_fig_data$country <- factor(ar_se_asia_fig_data$country,
                                         levels = country_list)

# Converting 'cause_of_death' to type factor
ar_se_asia_fig_data$cause_of_death <- factor(ar_se_asia_fig_data$cause_of_death, levels = diseases_list)

# wrapping x-axis labels text
levels(ar_se_asia_fig_data$cause_of_death) <- str_wrap(levels(ar_se_asia_fig_data$cause_of_death), 30)

# reorder within each location as per the total life years lost column
ar_se_asia_fig_data <- ar_se_asia_fig_data %>%
...
ar_se_asia_fig <- ar_se_asia_fig_data %>%
