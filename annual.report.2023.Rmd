...
ar_global_fig2.1_data <- gbd_results_master_2021 %>%
...
colnames(ar_global_fig2.1_data)[3] <- c("llpp_who_2021")

ar_global_fig2.1_data <- ar_global_fig2.1_data %>%
...
ar_global_fig2.1 <- ar_global_fig2.1_data %>%
  ggplot() + 
...
# creating a central and west africa specific dataset
...
# central and west africa trendlines 1998 to 2021 
fig4.3_data_part1 <- gadm2_aqli_2021_cw_african %>%
  mutate(region_full = "Central and West Africa") %>%
  ...
  pivot_longer(cols = pm1998_weighted:pm2021_weighted, names_to = "years", 
               values_to = "pop_weighted_avg_pm2.5") %>%
  mutate(years = as.integer(unlist(str_extract(years, "\\d+")))) %>%
  select(years, region_full, pop_weighted_avg_pm2.5)


# Africa trendlines 1998 to 2021

fig4.3_data_part2 <- gadm2_aqli_2021 %>%
  filter(continent == "Africa") %>%
  mutate(region_full = "Africa") %>%
  group_by(region_full) %>%
  filter(!is.na(population)) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE), 
         mutate(across(starts_with("pm"), ~.x*pop_weights, .names = "{col}_weighted"))) %>%
...
fig4.3_data_final <- fig4.3_data_part1 %>%
  rbind(fig4.3_data_part2)

fig4.3 <- fig4.3_data_final %>%
  mutate(region_full_order = ifelse(region_full == "Central and West Africa", 1, 2)) %>%
  ggplot() +
  geom_line(mapping = ggplot2::aes(x = years, y = pop_weighted_avg_pm2.5, color = forcats::fct_reorder(region_full, region_full_order)), lwd = 1.3) +
    geom_hline(mapping = aes(yintercept = 5), lwd = 0.8, linetype = "dotted") +
  scale_y_continuous(breaks = seq(0, 30, 5), limits = c(0, 30)) +
  scale_x_continuous(breaks = c(seq(1998, 2019, 3), 2021)) +
    scale_color_manual(values = c("Central and West Africa" = "#7197be", "Africa" = "#82b5d5")) +
  ggthemes::theme_tufte() +
  labs(x = "Year", 
  ...
  theme(legend.position = "bottom",
        ...
        axis.ticks = element_blank()) +
     geom_text(x = 2000.5, y = 5.8, label = expression("WHO" ~ PM[2.5] ~ "Guideline (last updated: 2021): 5 µg/m³"), size = 4.5) 
...
# chart5 dataset v2 (ViT colors and buckets): join colormap and USA county level shape file and adding a grouping column
ar_fig7.2_data_v2 <- colormap_shp_usa %>%
  left_join(color_data_usa, by = c("country", "name_1", "name_2")) %>%
  mutate(pollution_category = ifelse(pm2021 >= 0 & pm2021 < 5, "0 - < 5", pm2021), 
         pollution_category = ifelse(pm2021 >= 5 & pm2021 < 10, "5 - < 10", pollution_category), 
         pollution_category = ifelse(pm2021 >= 10 & pm2021 < 15, "10 - < 15", pollution_category), 
...
ar_fig7.2_v2 <- ggplot(data = ar_fig7.2_data_v2) +
  geom_sf(mapping = aes(fill = fct_reorder(pollution_category, order_pollution_category)), color = "aliceblue") + 
  geom_sf(data = gadm1_aqli_2021_shp %>% filter(name0 == "United States", name1 != "Alaska", name1 != "Hawaii"), fill = "transparent", color = "white", lwd = 1) +
   geom_sf(data = gadm1_aqli_2021_shp %>% filter(name0 == "United States", name1 == "California", name1 != "Alaska", name1 != "Hawaii"), fill = "transparent", color = "white", lwd = 1) +
  ggthemes::theme_map() +
  labs(fill = expression("Annual Average" ~ PM[2.5] ~ " Concentration (in  µg/m³)"), title = "") +
  scale_fill_manual(values = c("0 - < 5" = "#a1f5ff", "5 - < 10" = "#92d4eb", "10 - < 15" = "#82b5d5", ">= 15" = "#7197be")) +
  # ggtitle(expression(paste("US 2020 PM2.5 concentrations, (in ", mu, "g","/", m^3, ")", ""))) +
  theme(plot.background = element_rect(colour = "white", fill = "white"), 
        legend.position = "bottom", 
        legend.justification = "center", 
...
europe_1998_2021_diff_data <- gadm2_aqli_2021_europe %>%
  filter(country %notin% exclude_countries) %>%
   mutate(lyl1998minus2021 = round((pm1998 - pm2021)*0.098, 2)) %>%
  left_join(gadm2_aqli_2021_shp, by = c("objectid_gadm2" = "obidgadm2")) %>%
  add_aqli_color_scale_buckets("lyldiff", "lyl1998minus2021") %>%
  select(-geometry, geometry) %>%
  st_as_sf()
...
mutate(across(starts_with("llpp_nat"), ~as.character(case_when(
  ...
  rename(Country = country, "PM2.5 concentration 2021 (in µg/m³)" = pm2021, 
         ...
         "Life Expectancy Gains (in years) from reducing PM2.5 from 2021 Concentrations to the WHO Guideline of 5 µg/m³" = llpp_who_2021, 
         "Life Expectancy Gains (in years) from reducing PM2.5 from 2021 Concentrations to the National Standard" = llpp_nat_2021) %>%  
write_csv("[August082023]Annual Report 2023 appendix table.csv")
...
#> AR national standards continent wise stats----------------------------
natstan_aqli_summary <-  natstan_aqli %>%
  filter(country != "Caspian Sea") %>%
  left_join(gadm0_aqli_2021 %>% select(country, continent), by = "country") %>%
  select(country, continent, aaqs_countries_unep_2021_report, natstandard_pm2.5_new_2023_report_micr_grm_cubic_meter_op1) %>%
  mutate(have_pm2.5_standard = ifelse(natstandard_pm2.5_new_2023_report_micr_grm_cubic_meter_op1 != "No national standard", 1, 0))

# continent wise summary for countries with some legislative instruments for air quality
natstan_aqli_summary %>%
  group_by(continent) %>%
  summarise(perc_countries_with_leg_aaqs = round((sum(aaqs_countries_unep_2021_report)/n())*100, 1), 
            total_countries_by_continent = n()) 

natstan_aqli_summary %>%
 mutate(continent = ifelse(country == "China", "China", continent)) %>%
 mutate(continent = ifelse(continent == "Asia", "Asia - China", continent)) %>%
  group_by(continent) %>%
  summarise(perc_countries_with_pm2.5_standard = round((sum(have_pm2.5_standard)/n())*100, 1)) 

  
#> Global section stats

    
#> Continent wise average pollution
  
gadm2_aqli_2021 %>%
  gadm_level_summary(c("continent"), c(2021), 10) %>%
  ungroup() %>%
  mutate(perc_of_world_pop = round((population/sum(population, na.rm = TRUE))*100, 3)) %>%
  select(continent, population, perc_of_world_pop, pm2021, llpp_who_2021, total_lyl_who_2021_millions) 
  write_csv("./continent_wise_pollution_lyl_population.csv")
    

#> Continent wise average pollution (Asia - China style)
...
gadm2_aqli_2021 %>%
  ...
  gadm_level_summary(c("continent"), c(2021), 10) %>%
  ungroup() %>%
  mutate(perc_of_world_pop = round((population/sum(population, na.rm = TRUE))*100, 4)) %>%
  select(continent, population, perc_of_world_pop, pm2021, llpp_who_2021, total_lyl_who_2021_millions) 
  write_csv("./[v1]continent_wise_pollution_lyl_population.csv")
  
#> Europe + US + Canada (combined)
gadm2_aqli_2021 %>%
  filter((country %in% european_countries$Country) | (country %in% c("United States", "Canada"))) %>%
  mutate(country = "foo") %>%
   gadm_level_summary(c("country"), c(2021), 10) 
 
#> North America (minus US + Canada)
gadm2_aqli_2021 %>%
  filter(continent == "North America") %>%
  filter(country %notin% c("United States", "Canada")) %>%
  mutate(country = "foo") %>%
   gadm_level_summary(c("country"), c(2021), 10)
...
natstan_aqli_summary %>%
 ...
 mutate(continent = ifelse(continent == "Asia", "Asia - China", continent)) %>%
  group_by(continent) %>%
  summarise(have_pm2.5_stan_count = sum(have_pm2.5_standard), num_countries = n(), perc_countries_with_pm2.5_standard = round((sum(have_pm2.5_standard)/n())*100, 1)) 
...
openaq_data <- openaq_data %>%
  mutate(fully_open_aq_data_exists = ifelse(((data_in_phys_units == "Y") & (transp_geog_scale_data_provided == "Y") 
                                            & (data_fine_temporal_scale == "Y") & (prog_access == "Y")) == TRUE, 1, 0)) 
openaq_data %>%
  group_by(continent) %>%
  summarise(num_countries_full_open_aqdata = sum(fully_open_aq_data_exists), 
            num_countries_in_continent = n())
...
# global average pollution 1998 to 2021
 trendlines_aqli_data <- gadm2_aqli_2021 %>%
      dplyr::filter(!is.na(population)) %>%
      mutate(country = "foo") %>%
      dplyr::group_by(country) %>%
      dplyr::mutate(pop_weights = population/sum(population, na.rm = TRUE),
    ...
    ar_prelim_fig3 <- trendlines_aqli_data %>%
     ggplot2::ggplot() +
...
# -x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-[NOT PART of the main AR] Rough calculations-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x-x

```{r eval=FALSE, include=FALSE}
country_wise_risk_summary_data <- gadm2_aqli_2021 %>%
  filter(!is.na(population)) %>%
  mutate(risk_cat = ifelse(pm2021 <= 5, "No/Minor", NA), 
         risk_cat = ifelse(((pm2021  > 5) & (pm2021 <= 10)), "Low", risk_cat), 
         risk_cat = ifelse(((pm2021  > 10) & (pm2021 <= 15)), "Moderate", risk_cat), 
         risk_cat = ifelse(((pm2021  > 15) & (pm2021 <= 25)), "High", risk_cat), 
         risk_cat = ifelse(((pm2021  > 25) & (pm2021 <= 35)), "Very high", risk_cat), 
         risk_cat = ifelse(((pm2021  > 35)), "Hazardous", risk_cat)) %>%
  mutate(risk_cat_order = ifelse(risk_cat == "Low", 1, NA), 
         risk_cat_order = ifelse(risk_cat == "Moderate", 2, risk_cat_order), 
         risk_cat_order = ifelse(risk_cat == "High", 3, risk_cat_order), 
         risk_cat_order = ifelse(risk_cat == "Very high", 4, risk_cat_order), 
         risk_cat_order = ifelse(risk_cat == "Hazardous", 5, risk_cat_order)) %>%
  group_by(country, risk_cat) %>%
  summarise(tot_pop = sum(population, na.rm = TRUE)) %>%
  ungroup() %>% 
  group_by(country) %>%
  mutate(perc_pop = round((tot_pop/sum(tot_pop))*100, 1), 
         tot_pop_country = sum(tot_pop, na.rm = TRUE)) %>%
  ungroup()

country_wise_risk_summary_data$risk_cat <- factor(country_wise_risk_summary_data$risk_cat, levels = c("Hazardous", "Very high", "High", "Moderate", "Low", "No/Minor"))

country_wise_risk_summary_data %>%
  filter(country %in% c("India", "Pakistan", "China", "Poland", "Nigeria", "Australia")) %>%
  ggplot(mapping = aes(x = fct_reorder(country, tot_pop_country), y = perc_pop, fill = risk_cat)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  viridis::scale_fill_viridis(discrete = TRUE, option = "rocket") +
  ggthemes::theme_clean() +
  labs(x = "Country", y = "Percent population") +
  theme(legend.position = "bottom", 
        legend.title = element_blank())
...
%>%
  summarise(lyl_total = sum(total_lyl_who_2021_millions, na.rm = TRUE))

gadm2_aqli_2021 %>%
  filter(country %in% se_asia_vec, pm2021 > 5) %>%
  summarise(tot_pop = sum(population, na.rm = TRUE))
...
gadm2_aqli_2021 %>%
  ...
  slice_max(pm2021, n = 60) %>%
  View()
  filter(name_1 %in% c("Hà Nội", "Jakarta Raya", "Mandalay")) 
