...
# Part-1: BTH, YRD, PRD gain in LE from 2014 to 2022
bth_yrd_prd_gain_le_part1 <- color_2022_china %>%
  group_by(region) %>%
  ...
  summarise(avg_pm2.5_2022 = sum(pm2022_pop_weighted, na.rm = TRUE), 
            ...
            diff_avg_pm2.5_2014_to_2022 = (avg_pm2.5_2014 - avg_pm2.5_2022), 
            change_in_le_2014_to_2022 = round((diff_avg_pm2.5_2014_to_2022*0.098), 1)) %>%
  filter(region %in% c("BTH", "YRD", "PRD"))

# Part-2: China gain in LE from 2014 to 2022
china_gain_le_part2 <- color_2022_china %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE), 
         pm2022_pop_weighted  = pm2022*pop_weights, 
         pm2014_pop_weighted = pm2014*pop_weights) %>% 
  summarise(avg_pm2.5_2022 = sum(pm2022_pop_weighted, na.rm = TRUE), 
...
# Part-3: China gain in LE for specific regions as shown in AR 
# regions of interest (note: there are 2 "Suzhou", I have taken the one that is 
# in the Jiangsu gadm level 1 region, mention this in a footnote or as a note  
...
custom_regions_china_le_part3 <- color_2022_china %>%
  filter(name_2 %in% gadm2_regions_to_plt) %>%
  group_by(name_1, name_2) %>%
  summarise(pm2014 = pm2014[1], pm2022 = pm2022[1], diff_avg_pm2.5_2014_to_2022 = (pm2014 - pm2022), 
            change_in_le_2014_to_2022 = round((diff_avg_pm2.5_2014_to_2022*0.098), 1)) %>%
  filter(name_1 != "Jiangsu") %>%
  ungroup() %>%
  select(name_2, pm2022, pm2014, diff_avg_pm2.5_2014_to_2022, change_in_le_2014_to_2022) %>%
  rename(region = name_2, avg_pm2.5_2022 = pm2022, avg_pm2.5_2014 = pm2014) 
...
ar_fig6.3 <- ar_fig6.3_data %>%
  add_aqli_color_scale_buckets(scale_type = "lyldiff", col_name = "change_in_le_2014_to_2022") %>%
  ...
  ggplot() +
  geom_col(mapping = aes(x = fct_reorder(region, order_manual), 
                         y = change_in_le_2014_to_2022, 
                         fill = forcats::fct_reorder(lyldiff_bucket, order_lyldiff_bucket))) +
