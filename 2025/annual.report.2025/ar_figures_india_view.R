...
# aksai_chin_xz <- st_difference(china_w_aksai_chin, china_without_aksai_chin)
#   
# aksai_chin_xu %>%
#   bind_rows(aksai_chin_xz) %>%
#   select(obidgadm1, name0, name1, geometry) %>%
#   st_write("~/Desktop/AQLI/shapefiles/aksai_chin/aksai_chin.shp")

#### India shapefiles ####
india_gadm0 <- st_read("~/Desktop/AQLI/shapefiles/India_Worldview/gadm0_India_IndWV/india_gadm0.shp")
...
azad_kashmir_gilgit_baltistan <- gadm2_aqli_shp %>%
  filter(name0 == "Pakistan", name1 %in% c("Azad Kashmir", "Gilgit Baltistan")) %>%
  mutate(name0 = "India",
         name1 = "Jammu and Kashmir")

india_gadm2 <- gadm2_aqli_shp %>%
  filter(name0 == "India") %>%
  bind_rows(azad_kashmir_gilgit_baltistan) %>%
  bind_rows(aksai_chin_gadm2)

# india_gadm2 %>% st_write("~/Desktop/AQLI/shapefiles/India/india_gadm2.shp")

# india_gadm0 <- india_state %>% st_union()
# india_gadm0 %>% st_write("~/Desktop/AQLI/shapefiles/India/india_gadm0.shp")

##### test #####
test <- india_gadm2 %>%
  ggplot() +
  geom_sf(color = "darkgrey", fill = "white", lwd = 0.05) +
  geom_sf(data = india_state, color = "black", fill = "transparent", lwd = 0.1) +
...
sa_wo_ind_pak <- gadm2_aqli_shp %>%
  filter(name0 %in% c("Afghanistan", "Bangladesh", "Bhutan", "Maldives", "Nepal", "Sri Lanka"))

pak_wo_pok <- gadm2_aqli_shp %>%
  filter(name0 == "Pakistan", name1 %notin% c("Azad Kashmir", "Gilgit Baltistan")) 

pak_wo_pok_gadm0 <- gadm2_aqli_shp %>% 
  filter(name0 == "Pakistan", name1 %notin% c("Azad Kashmir", "Gilgit Baltistan")) %>%
  st_union()

south_asia_gadm2_shp <- india_gadm2 %>%
  bind_rows(sa_wo_ind_pak) %>%
  bind_rows(pak_wo_pok)

south_asia_gadm1_shp <- india_state %>%
  bind_rows(gadm2_aqli_shp %>% filter(name0 == "Pakistan", name1 %notin% c("Azad Kashmir", "Gilgit Baltistan"))) %>%
  bind_rows(gadm2_aqli_shp %>% filter(name0 %in% c("Afghanistan", "Bangladesh", "Bhutan", "Maldives", "Nepal", "Sri Lanka")))

sa_wo_ind_pak_gadm0 <- gadm2_aqli_shp %>% 
  filter(name0 %in% c("Afghanistan", "Bangladesh", "Bhutan", "Maldives", "Nepal", "Sri Lanka"))
...
ar_south_asia_fs_fig5.2 <- ar_south_asia_fs_fig5.2_data %>%
  ...
  geom_sf(data = pak_wo_pok_gadm0, color = "cornsilk4", fill = "transparent", lwd = 0.3) +
  geom_sf(data = india_gadm0, color = "cornsilk4", fill = "transparent", lwd = 0.3) +
  geom_sf(data = sa_wo_ind_pak_gadm0, color = "cornsilk4", fill = "transparent", lwd = 0.3) +
  ggthemes::theme_map() +
...
#### China Figures #### 
# turning off spherical geometry to avoid duplicate vertices
# sf_use_s2(FALSE)

# china_wo_aksaichin_gadm0 <- china_wo_aksaichin_gadm1 %>%
#   st_union()
# china_wo_aksaichin_gadm0 %>% st_write("~/Desktop/AQLI/shapefiles/India_worldview/China/china_gadm0.shp")

china_wo_aksaichin_gadm1 <- st_read("~/Desktop/AQLI/shapefiles/India_Worldview/gadm2_China_IndWV/china_state_cut.shp")
china_wo_aksaichin_gadm0 <- st_read("~/Desktop/AQLI/shapefiles/India_Worldview/gadm0_China_IndWV/china_gadm0.shp")

china_w_aksai_chin_gadm2 <- gadm2_aqli_shp %>% filter(name0 == "China")

china_wo_aksaichin_gadm2 <- st_intersection(china_wo_aksaichin_gadm1, china_w_aksai_chin_gadm2) %>%
  select(obidgadm2, name0.1, name1.1, name2, geometry) %>%
  rename(name0 = name0.1,
         name1 = name1.1)

##### Test #####
test2 <- china_wo_aksaichin_gadm2 %>%
  ggplot() +
  geom_sf(color = "darkgrey", fill = "white", lwd = 0.05) +
  geom_sf(data = china_wo_aksaichin_gadm1, color = "black", fill = "transparent", lwd = 0.1) +
  geom_sf(data = china_wo_aksaichin_gadm0, color = "cornsilk4", fill = "transparent", lwd = 0.3)

ggsave("test2.png", test2, width = 15, height = 10)

##### Figure 6.1 ##### 
# filter data for china and create shapefile
ar_china_fig6.1_data <- gadm2_aqli_2023 %>%
  mutate(who2023 = ifelse(pm2022 <= 5, 0, who2023)) %>%
  inner_join(china_wo_aksaichin_gadm2, by = c("id" = "obidgadm2")) %>%
  add_aqli_color_scale_buckets("lyl", "who2023") %>%
  select(-geometry, geometry) %>%
  st_as_sf()
...
ar_china_fig6.1 <- ar_china_fig6.1_data %>%
  ggplot() +
  geom_sf(mapping = aes(fill = forcats::fct_reorder(lyl_bucket, order_lyl_bucket)), color = "aliceblue", lwd = 0.05) +
  geom_sf(data = china_wo_aksaichin_gadm1, color = "azure4", fill = "transparent", lwd = 0.1) +
  geom_sf(data = china_wo_aksaichin_gadm0, color = "cornsilk4", fill = "transparent", lwd = 0.3) +
  ggthemes::theme_map() +
  scale_fill_manual(values = c("0 to < 0.1" = "#FFFFFF", 
                               "0.1 to < 0.5" = "#FFF2E1", 
...
ar_china_fig6.2 <- ar_china_fig6.2_data %>%
  ...
  geom_sf(data = china_wo_aksaichin_gadm1, color = "azure4", fill = "transparent", lwd = 0.15) +
  geom_sf(data = china_wo_aksaichin_gadm0, color = "cornsilk4", fill = "transparent", lwd = 0.5) +
  ggthemes::theme_map() + 
...
#### Global Figures ####
# country_shape_file_india_wv_1 <- gadm0_aqli_2022_shp %>%
#   filter(name0 %notin% c("China", "India", "Pakistan"))
# 
...
# 
# china_gadm0 <- st_read("~/Desktop/AQLI/shapefiles/China_country_IndWV/china_gadm0.shp") %>%
#   mutate(name0 = "China",
#          obidgadm0 = 46,
#          isoalp3 = "CHN") %>%
#   select(obidgadm0, isoalp3, name0, geometry)
#
# pak_gadm0 <- st_read("~/Desktop/AQLI/shapefiles/Pakistan_country_IndWV/pak_gadm0.shp") %>%
#   mutate(name0 = "Pakistan",
#          obidgadm0 = 166,
#          isoalp3 = "PAK") %>%
#   select(obidgadm0, isoalp3, name0, geometry)
# 
# country_shape_file_india_wv <- country_shape_file_india_wv_1 %>%
#   bind_rows(india_gadm0) %>%
#   bind_rows(china_gadm0) %>%
#   bind_rows(pak_gadm0)

# country_shape_file_india_wv %>% st_write("~/Desktop/AQLI/shapefiles/gadm0_india_worldview/country_gadm0.shp")

country_shape_file_india_wv <- st_read("~/Desktop/AQLI/shapefiles/India_Worldview/gadm0_IndWV/country_gadm0.shp")

test3 <- country_shape_file_india_wv %>%
  ggplot() +
...
ar_global_fig2.1_data <- monitor_density %>%
  ...
  inner_join(country_shape_file_india_wv, by = c("name" = "name0")) %>%
  select(-geometry, geometry, ) %>%
  st_as_sf()
...
ar_global_fig2.1 <- ggplot() +
  ...
  geom_sf(data = country_shape_file_india_wv, color = "cornsilk4", fill = "transparent", lwd = 0.3) +
  ggthemes::theme_map() +
...
ar_global_fig2.3_data <- awardees %>%
  ...
  inner_join(country_shape_file_india_wv, by = c("country" = "name0")) %>%
  select(-geometry, geometry) %>%
  st_as_sf()
...
ar_global_fig2.3 <- ggplot() +
  geom_sf(data = ar_global_fig2.3_data, 
          ...
          color = "white", 
          lwd = 0.3) + 
  geom_sf(data = country_shape_file_india_wv, 
          color = "cornsilk4", 
          fill = "transparent", 
          lwd = 0.3) +
