...
ar_oceania_fig11.1_data <- gbd_results_master_2025 %>%
  filter(cause_of_death %in% diseases_list, 
         country %in% country_list)

# making the 'location' column of type factor
ar_oceania_fig11.1_data$country <- factor(ar_oceania_fig11.1_data$country,
                                       levels = country_list)

# Converting 'cause_of_death' to type factor
ar_oceania_fig11.1_data$cause_of_death <- factor(ar_oceania_fig11.1_data$cause_of_death, levels = diseases_list)

# wrapping x-axis labels text
levels(ar_oceania_fig11.1_data$cause_of_death) <- str_wrap(levels(ar_oceania_fig11.1_data$cause_of_death), 30)

# reorder within each location as per the total life years lost column
ar_oceania_fig11.1_data <- ar_oceania_fig11.1_data %>%
...
# plot
ar_oceania_fig11.1 <- ar_oceania_fig11.1_data %>%
