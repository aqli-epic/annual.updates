...
# Figure 3: Top 10 threats to life expectancy in Cameroon

gbd_results_cam <- gbd_results_master_2025  %>%
  filter(cause_of_death %in% c("Ambient ozone pollution","Child and maternal malnutrition","Childhood sexual abuse and bullying",
                               ...
                               "Tobacco","Transport injuries","Non-optimal temperature","Other environmental risks","Unintentional injuries","Unsafe sex","Unsafe water, sanitation, and handwashing"),country == "Cameroon")

# filtering out those causes of death that are sort of* covered under PM2.5 in some broad way
colnames(gbd_results_cam)[3] <- c("llpp_who_2023")

cam_fs_fig3_dataset <- gbd_results_cam %>%
  add_aqli_color_scale_buckets("lyl", "llpp_who_2023") %>%
  slice_max(llpp_who_2023, n = 10)

# cam factsheet figure 3
cam_fs_fig3 <- cam_fs_fig3_dataset %>%
  ggplot() +
