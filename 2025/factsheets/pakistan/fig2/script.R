...
#Figure 2:  Potential gain in life expectancy from reducing PM2.5 from 2023 levels to the WHO guideline in all provinces of Pakistan
...
pak_fs_fig2_dataset <- pak_aqli_2023 %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2023', 'llpp_who_2023', 'llpp_nat_2023') %>%
  group_by(name_1) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         pm2023_pop_weighted = pop_weights*pm2023,
         llpp_who_2023_pop_weighted = pop_weights*llpp_who_2023,
         llpp_nat_2023_pop_weighted = pop_weights*llpp_nat_2023) %>%
  summarise(tot_pop = sum(population, na.rm = TRUE),
            ...
            llpp_nat_2023 = sum(llpp_nat_2023_pop_weighted, na.rm = TRUE),
