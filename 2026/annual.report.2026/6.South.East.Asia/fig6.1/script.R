...
ar_se_asia_fig_data <- gadm2_aqli_2024 %>%
...
ar_se_asia_fig <- ar_se_asia_fig_data %>%
  ...
  geom_sf(data = gadm1_aqli_2024_shp %>% filter(name0 %in% se_asia_vec), color = "azure4", fill = "transparent", lwd = 0.1) +
  geom_sf(data = gadm0_aqli_2024_shp %>% filter(name0 %in% se_asia_vec), color = "cornsilk4", fill = "transparent", lwd = 0.3) +
  ggthemes::theme_map() +
