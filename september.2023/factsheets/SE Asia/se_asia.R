...
# read shapefiles and filter for se_asia
...
# read and filter gbd results for se_asia
gbd_results_se_asia <- read.csv(glue("{dir}/gbd_results_master.csv")) %>%
  filter(country %in% se_asia_countries)

# se_asia fs fig 1 data
se_asia_fs_fig1_dataset <- aqli_2021 %>%
...
# save se_asia fs figure 1
ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig1.png"), height = 10, width = 15, se_asia_fs_fig1)

svglite("se_asia_fs_fig1")
ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig1.svg"), height = 10, width = 15, dpi = 300, se_asia_fs_fig1)

# se_asia fs fig 2 data
se_asia_fs_fig2_dataset <- aqli_2021 %>%
...
# save figure 2 data
# se_asia_fs_fig2_dataset %>% write_csv(glue("{dir}/SE Asia/output/se_asia_fs_fig2_dataset.csv"))
...
# save se_asia fs figure 2
ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig2.png"), height = 9, width = 12, se_asia_fs_fig2)

svglite("se_asia_fs_fig2")
ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig2.svg"), height = 9, width = 12, dpi = 300, se_asia_fs_fig2)

# # filtering out those causes of death that are sort of* covered under PM2.5 in some broad way
# colnames(gbd_results_se_asia)[3] <- c("llpp_who_2021")
# 
# se_asia_fs_fig3_dataset <- gbd_results_se_asia %>%
#   add_aqli_color_scale_buckets("lyl", "llpp_who_2021") %>%
#   slice_max(llpp_who_2021, n = 10)
# 
# #save figure 3 data
# se_asia_fs_fig3_dataset %>% write_csv(glue("{dir}/SE Asia/output/se_asia_fs_fig3_dataset.csv"))
# 
# # se_asia factsheet figure 3
# se_asia_fs_fig3 <- se_asia_fs_fig3_dataset %>%
#   ggplot() + 
...
# 
# # save se_asia fs figure 3
# ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig3.png"), height = 9, width = 15, se_asia_fs_fig3)
# 
# svglite("se_asia_fs_fig3")
# ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig3.svg"), height = 9, width = 15, se_asia_fs_fig3)

# se_asia factsheet figure 4 dataset
se_asia_fs_fig4_dataset <- aqli_2021 %>%
  filter(name_2 != "Kalibato Lake") %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
...
se_asia_fs_fig4_dataset <- aqli_2021 %>%
  ...
  select(years, region, pop_weighted_avg_pm2.5)
# save figure 4 data
# se_asia_fs_fig4_dataset %>% write_csv(glue("{dir}/SE Asia/output/se_asia_fs_fig4_dataset.csv"))
...
# save se_asia fs figure 4
ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig4.png"), height = 9, width = 15, se_asia_fs_fig4)

svglite("se_asia_fs_fig4")
ggsave(glue("{dir}/SE Asia/output/se_asia_fs_fig4.svg"), height = 9, width = 15, dpi = 300, se_asia_fs_fig4)

# se_asia factsheet appendix table 
se_asia_fs_appendix_table <- aqli_2021 %>%
  group_by(country) %>%
...
# save se_asia factsheet appendix table
se_asia_fs_appendix_table %>% write_csv(glue("{dir}/SE Asia/output/se_asia_fs_appendix_table.csv"))

# other se asia factsheet stats
aqli_2021_gadm1 <- read.csv(glue("{dir}/[june302023]gadm1_aqli_2021_internal_col_names.csv")) %>%
  filter(country %in% se_asia_countries)  %>%
  mutate(name_1_country = str_c(name_1, " (", country, ")", sep = ""))

aqli_2021_gadm0 <- read.csv(glue("{dir}/[june302023]gadm0_aqli_2021_internal_col_names.csv")) %>%
  filter(country %in% se_asia_countries)

# diagnostics
filter(aqli_2021_2, !complete.cases(across(starts_with("pm"))))
