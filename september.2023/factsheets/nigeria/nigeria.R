...
# read shapefiles and filter for nigeria
...
gbd_results_nigeria <- read.csv(glue("{dir}/gbd_results_master.csv")) %>%
  filter(country == "Nigeria")

# nigeria fs fig 1 data
nigeria_fs_fig1_dataset <- aqli_2021 %>%
...
# save nigeria fs figure 1
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig1.png"), height = 10, width = 15, nigeria_fs_fig1)

svglite("nigeria_fs_fig1")
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig1.svg"), height = 10, width = 15, dpi = 300, nigeria_fs_fig1)

# nigeria fs fig 2 data
nigeria_fs_fig2_dataset <- aqli_2021 %>%
...
# save figure 2 data
# nigeria_fs_fig2_dataset %>% write_csv(glue("{dir}/nigeria/output/nigeria_fs_fig2_dataset.csv"))
...
# save nigeria fs figure 2
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig2.png"), height = 9, width = 12, nigeria_fs_fig2)

svglite("nigeria_fs_fig2")
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig2.svg"), height = 9, width = 12, dpi = 300, nigeria_fs_fig2)
...
# save figure 3 data
# nigeria_fs_fig3_dataset %>% write_csv(glue("{dir}/nigeria/output/nigeria_fs_fig3_dataset.csv"))
...
# save nigeria fs figure 3
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig3.png"), height = 9, width = 15, nigeria_fs_fig3)

svglite("nigeria_fs_fig3")
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig3.svg"), height = 9, width = 15, dpi = 300, nigeria_fs_fig3)

# nigeria factsheet figure 4 dataset
nigeria_fs_fig4_dataset <- aqli_2021 %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
...
# save figure 4 data
# nigeria_fs_fig4_dataset %>% write_csv(glue("{dir}/nigeria/output/nigeria_fs_fig4_dataset.csv"))
...
# save nigeria fs figure 4
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig4.png"), height = 9, width = 15, nigeria_fs_fig4)

svglite("nigeria_fs_fig4")
ggsave(glue("{dir}/nigeria/output/nigeria_fs_fig4.svg"), height = 9, width = 15, dpi = 300, nigeria_fs_fig4)

# nigeria factsheet appendix table 
nigeria_fs_appendix_table <- aqli_2021 %>%
...
nigeria_fs_appendix_table %>% write_csv(glue("{dir}/nigeria/output/nigeria_fs_appendix_table.csv"))

# other factsheet calculations
nigeria_niger_delta <- aqli_2021 %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021') %>%
  filter(name_1 %in% c("Abia", "Akwa Ibom", "Bayelsa", "Cross River", "Delta", 
                       "Edo", "Imo", "Ondo", "Rivers")) %>%
  group_by(name_1) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         pm2021_pop_weighted = pop_weights*pm2021,
...
nigeria_most_polluted_states <- aqli_2021 %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021') %>%
  group_by(name_1) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         pm2021_pop_weighted = pop_weights*pm2021,
