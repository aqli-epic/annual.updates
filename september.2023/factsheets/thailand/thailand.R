...
# read shapefiles and filter for thailand
...
gbd_results_thailand <- read.csv(glue("{dir}/gbd_results_master.csv")) %>%
  filter(country == "Thailand")

# thailand fs fig 1 data
thailand_fs_fig1_dataset <- aqli_2021 %>%
...
# save thailand fs figure 1
ggsave(glue("{dir}/thailand/output/thailand_fs_fig1.png"), height = 10, width = 15, thailand_fs_fig1)

svglite("thailand_fs_fig1")
ggsave(glue("{dir}/thailand/output/thailand_fs_fig1.svg"), height = 10, width = 15, dpi = 300, thailand_fs_fig1)

# thailand fs fig 2 data
thailand_fs_fig2_dataset <- aqli_2021 %>%
...
# save figure 2 data
# thailand_fs_fig2_dataset %>% write_csv(glue("{dir}/thailand/output/thailand_fs_fig2_dataset.csv"))
...
# save thailand fs figure 2
ggsave(glue("{dir}/thailand/output/thailand_fs_fig2.png"), height = 9, width = 12, thailand_fs_fig2)

svglite("thailand_fs_fig2")
ggsave(glue("{dir}/thailand/output/thailand_fs_fig2.svg"), height = 9, width = 12, dpi = 300, thailand_fs_fig2)
...
# save figure 3 data
# thailand_fs_fig3_dataset %>% write_csv(glue("{dir}/thailand/output/thailand_fs_fig3_dataset.csv"))
...
# save thailand fs figure 3
ggsave(glue("{dir}/thailand/output/thailand_fs_fig3.png"), height = 9, width = 15, thailand_fs_fig3)

svglite("thailand_fs_fig3")
ggsave(glue("{dir}/thailand/output/thailand_fs_fig3.svg"), height = 9, width = 15, dpi = 300, thailand_fs_fig3)

# thailand factsheet figure 4 dataset
thailand_fs_fig4_regions <- aqli_2021 %>%
  group_by(region) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE), 
...
thailand_fs_fig4_nat <- aqli_2021 %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         mutate(across(starts_with("pm"), ~.x*pop_weights, .names = "{col}_weighted"))) %>%
  summarise(across(ends_with("weighted"), sum)) %>%
  pivot_longer(cols = pm1998_weighted:pm2021_weighted, names_to = "years",
...
# save figure 4 data
# thailand_fs_fig4_dataset %>% write_csv(glue("{dir}/thailand/output/thailand_fs_fig4_dataset.csv"))
...
# save thailand fs figure 4
ggsave(glue("{dir}/thailand/output/thailand_fs_fig4.png"), height = 9, width = 15, thailand_fs_fig4)

svglite("thailand_fs_fig4")
ggsave(glue("{dir}/thailand/output/thailand_fs_fig4.svg"), height = 9, width = 15, dpi = 300, thailand_fs_fig4)

# thailand factsheet appendix table 
thailand_fs_appendix_table <- aqli_2021 %>%
...
# save thailand factsheet appendix table
thailand_fs_appendix_table %>% write_csv(glue("{dir}/thailand/output/thailand_fs_appendix_table.csv"))

# other thailand factsheet calculations
se_asia_countries <-  c("Brunei", "Myanmar", "Cambodia", "Timor-Leste", "Indonesia", 
                        "Laos", "Malaysia", "Philippines", "Singapore", "Thailand", 
                        "Vietnam")

se_asia_gadm0 <- read.csv(glue("{dir}/[june302023]gadm1_aqli_2021_internal_col_names.csv"))  %>%
  filter(country %in% se_asia_countries)

aqli_gadm1 <- read.csv(glue("{dir}/[june302023]gadm1_aqli_2021_internal_col_names.csv")) %>%
  filter(country == "Thailand")
