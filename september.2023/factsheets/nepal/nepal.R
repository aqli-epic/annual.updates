...
# read shapefiles and filter for Nepal
...
gbd_results_nepal <- read.csv(glue("{dir}/gbd_results_master.csv")) %>%
  filter(country == "Nepal")

# nepal fs fig 1 data
nepal_fs_fig1_dataset <- aqli_2021 %>%
...
# save nepal fs figure 1
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig1.png"), height = 10, width = 15, nepal_fs_fig1)

svglite("nepal_fs_fig1")
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig1.svg"), height = 10, width = 15, dpi = 300, nepal_fs_fig1)

# nepal fs fig 2 data
nepal_fs_fig2_dataset <- aqli_2021 %>%
...
# save figure 2 data
# nepal_fs_fig2_dataset %>% write_csv(glue("{dir}/Nepal/output/nepal_fs_fig2_dataset.csv"))
...
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig2.png"), height = 9, width = 12, nepal_fs_fig2)

svglite("nepal_fs_fig2")
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig2.svg"), height = 9, width = 12, dpi = 300, nepal_fs_fig2)
...
nepal_fs_fig3_dataset <- gbd_results_nepal %>%
  add_aqli_color_scale_buckets("lyl", "llpp_who_2021") %>%
  slice_max(llpp_who_2021, n = 5)

# save figure 3 data
# nepal_fs_fig3_dataset %>% write_csv(glue("{dir}/Nepal/output/nepal_fs_fig3_dataset.csv"))
...
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig3.png"), height = 9, width = 15, nepal_fs_fig3)

svglite("nepal_fs_fig3")
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig3.svg"), height = 9, width = 15, dpi = 300, nepal_fs_fig3)

# nepal factsheet figure 4 dataset
nepal_fs_fig4_dataset <- aqli_2021 %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
...
# save figure 4 data
# nepal_fs_fig4_dataset %>% write_csv(glue("{dir}/Nepal/output/nepal_fs_fig4_dataset.csv"))
...
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig4.png"), height = 9, width = 15, nepal_fs_fig4)

svglite("nepal_fs_fig4")
ggsave(glue("{dir}/Nepal/output/nepal_fs_fig4.svg"), height = 9, width = 15, dpi = 300, nepal_fs_fig4)

# nepal factsheet appendix table 
nepal_fs_appendix_table <- aqli_2021 %>%
...
nepal_fs_appendix_table %>% write_csv(glue("{dir}/nepal/output/nepal_fs_appendix_table.csv"))
...
nepal_avg <- aqli_2021 %>%
  ...
  select(name_2, pop_millions, pm2021, le_gain_red_to_who, 
         le_gain_red_by_30_percent) %>%
  ungroup() %>%
  filter(name_2 %in% c("Kapilbastu", "Rupandehi", "Nawalparasi West", "Parsa", "Bara", 
                    "Rautahat", "Sarlahi", "Mahottari", "Dhanusha", "Siraha", "Saptari",
                    "Nawalparasi East", "Sunsari", "Morang", "Jhapa"))
