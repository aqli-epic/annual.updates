...
china_fs_fig1.1_data <- gadm2_aqli_2021 %>%
  filter(country == "China") %>%
...
# china figure 1
china_fs_fig1.1 <- china_fs_fig1.1_data %>%
  ggplot() +
...
gbd_results_europe_fig4 <- gbd_results_master_2021 %>%
  filter(cause_of_death %in% diseases_list, country %in% country_list)

# making the 'location' column of type factor
gbd_results_europe_fig4$country <- factor(gbd_results_europe_fig4$country, 
                                        levels = country_list)
...
# creating region wise average PM2.5 data from 1998 to 2021 
europe_fs_fig5_data <- gadm2_aqli_2021_europe %>%
  filter(!is.na(population)) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE), 
         mutate(across(starts_with("pm"), ~.x*pop_weights, .names = "{col}_weighted"))) %>%
  summarise(across(ends_with("weighted"), sum)) %>%
...
# fig 5
europe_fs_fig5 <- europe_fs_fig5_data %>%
  ggplot() +
...
appendix_table_final %>%
  write_csv("./[July052023]EuropeFactsheet2023_appendix_table.csv")
...
indonesia_fs_fig1_dataset <- gadm_aqli_2021 %>%
  left_join(gadm2_aqli_2021, by = c("objectid_gadm2" = "obidgadm2")) %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021', 'geometry') %>%
  add_aqli_color_scale_buckets("lyl", "llpp_who_2021") %>%
  select(-geometry, geometry) %>%
...
indonesia_fs_fig4_dataset <- gadm_aqli_2021 %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         mutate(across(starts_with("pm"), ~.x*pop_weights, .names = "{col}_weighted"))) %>%
  summarise(across(ends_with("weighted"), sum)) %>%
  pivot_longer(cols = pm1998_weighted:pm2021_weighted, names_to = "years",
...
indonesia_fs_appendix_table <- gadm_aqli_2021 %>%
  mutate(pm2021 = round(pm2021, 1),
         le_gain_red_to_who = round((pm2021 - who_guideline)*le_constant, 1),
         le_gain_red_to_who = ifelse(le_gain_red_to_who < 0, 0, le_gain_red_to_who),
...
# nepal fs fig 1 data
nepal_fs_fig1_dataset <- gadm_aqli_2021 %>%
  left_join(gadm2_aqli_2021, by = c("objectid_gadm2" = "obidgadm2")) %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021', 'geometry') %>%
  add_aqli_color_scale_buckets("lyl", "llpp_who_2021") %>%
  select(-geometry, geometry) %>%
...
# nepal fs fig 2 data
nepal_fs_fig2_dataset <- gadm_aqli_2021 %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021') %>%
  group_by(name_1) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         pm2021_pop_weighted = pop_weights*pm2021,
...
nepal_fs_fig4_dataset <- gadm_aqli_2021 %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         mutate(across(starts_with("pm"), ~.x*pop_weights, .names = "{col}_weighted"))) %>%
  summarise(across(ends_with("weighted"), sum)) %>%
  pivot_longer(cols = pm1998_weighted:pm2021_weighted, names_to = "years",
...
nigeria_fs_fig1_dataset <- gadm_aqli_2021 %>%
  left_join(gadm2_aqli_2021, by = c("objectid_gadm2" = "obidgadm2")) %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021', 'geometry') %>%
  add_aqli_color_scale_buckets("lyl", "llpp_who_2021") %>%
  select(-geometry, geometry) %>%
...
nigeria_fs_fig2_dataset <- gadm_aqli_2021 %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021') %>%
  group_by(name_1) %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         pm2021_pop_weighted = pop_weights*pm2021,
...
nigeria_fs_fig4_dataset <- gadm_aqli_2021 %>%
  mutate(pop_weights = population/sum(population, na.rm = TRUE),
         mutate(across(starts_with("pm"), ~.x*pop_weights, .names = "{col}_weighted"))) %>%
  summarise(across(ends_with("weighted"), sum)) %>%
  pivot_longer(cols = pm1998_weighted:pm2021_weighted, names_to = "years",
...
nigeria_fs_appendix_table <- gadm_aqli_2021 %>%
  mutate(pm2021 = round(pm2021, 1),
         le_gain_red_to_who = round((pm2021 - who_guideline)*le_constant, 1),
         le_gain_red_to_who = ifelse(le_gain_red_to_who < 0, 0, le_gain_red_to_who),
         le_gain_red_by_30_percent = round((pm2021*0.3)*le_constant, 1), 
...
pak_aqli_2021 <- read.csv(glue("{dir}/[June302023]aqli_internal_col_names_CSVs/[june302023]gadm2_aqli_2021_internal_col_names.csv")) %>%
...
se_asia_fs_fig1_dataset <- gadm_aqli_2021 %>%
  left_join(gadm2_aqli_2021, by = c("objectid_gadm2" = "obidgadm2")) %>%
  select('country', 'name_1', 'name_2', 'population', 'pm2021', 'llpp_who_2021', 'geometry') %>%
  add_aqli_color_scale_buckets("lyl", "llpp_who_2021") %>%
  select(-geometry, geometry) %>%
...
se_asia_fs_fig2_dataset <- gadm_aqli_2021 %>%
...
se_asia_fs_fig4_dataset <- gadm_aqli_2021 %>%
...
se_asia_fs_appendix_table <- gadm_aqli_2021 %>%
  group_by(country) %>%
  slice_max(population, n=5) %>%
  mutate(pm2021 = round(pm2021, 1),
         le_gain_red_to_who = round((pm2021 - who_guideline)*le_constant, 1),
...
thai_aqli_2021 <- read.csv(glue("{dir}/[June302023]aqli_internal_col_names_CSVs/[june302023]gadm2_aqli_2021_internal_col_names.csv")) %>%
...
us_fs_fig3_final_data <- us_fs_fig3_dataset_part1 %>%
  ...
  mutate(lyg_2021_1970 = round((pm2021 - pm1970)*0.098, 1)) %>%
  add_aqli_color_scale_buckets(scale_type = "lyldiff", "lyg_2021_1970")

# figure 3
us_fs_fig3 <- us_fs_fig3_final_data %>%
  filter(name_1 != "Georgia") %>%
   ggplot() +
      geom_col(mapping = aes(x = forcats::fct_reorder(!!as.symbol("name_1"), !!as.symbol("lyg_2021_1970")), y = !!as.symbol("lyg_2021_1970"), fill = lyldiff_bucket)) +
      labs(x = "State", y = "Change in potential gains in life expectancy between 1970 and 2021 (Years)", title = "", subtitle = "", caption = "", fill = "Change in potential gains in life expectancy between 1970 and 2021 (Years)") +   
  themes_aqli_base + 
   scale_fill_manual(values = c("< -2" = "#4575b4", 
                               "-2 to (< -0.5)" = "#74add1", 
                               "-0.5 to (< -0.1)" = "#abd9e9", 
