...
china_fs_fig3 <- china_fs_fig3_dataset %>%
  ...
  theme(legend.position = "bottom", legend.title = element_blank(), 
        ...
        axis.title.y = element_text(size = 9), 
        axis.title.x = element_text(size = 9)) + 
    geom_text(x = 2001, y = 8.5, label = expression(paste("WHO PM2.5 Guideline (last updated: 2021): 5 ", mu, "g","/", m^3, ""))) + 
...
#> plot 1: 2 regions graph, Indo-Gangetic plain and rest of India----------------------------------

plt <- india_state %>%
    mutate(region = ifelse(NAME_1 %in% north_india, "Indo-Gangetic Plain", "Other States and UTs")) %>%
    select(-geometry, geometry) %>%
    st_as_sf() %>%
    ggplot() +
    geom_sf(mapping = aes(fill = region), lwd = 0.8) +
    scale_fill_manual(values = c("Indo-Gangetic Plain" = "slategray2", "Other States and UTs" = "lightgrey")) +
    theme_map() +
    labs(fill = "") + 
    theme(legend.position = "bottom")
...
p#> plot 2: AQLI map screenshot (I have limited this to the Indo-Gangetic plain as opposed to the current screenshot that shows it for all of India)------------------
...
north_india_fs_fig2 <- north_india_fs_fig3_dataset %>%
  aqli_bar(scale_type = "lyl", x_var = "name_2", y_var = "llpp_who_2021", title = expression("Potential Gain in Life Expectancy from Reducing 2021 " ~ PM[2.5] ~ "concentrations to the WHO Guideline"), subtitle = "(Top 10 most populous districts of North India)", x_label = "District", y_label = "Average Life Expectancy Gains (years)", legend_title = "LE gains (years)", caption = "") +
...
north_india_fs_fig4_data_part1 <- gadm2_aqli_2021 %>%
  filter(country == "India") %>%
  mutate(region = ifelse(name_1 %in% north_india, "Indo-Gangetic Plain", "All other regions (excluding Indo-Gangetic Plain)"))
...
north_india_fs_fig4_data$region <- factor(north_india_fs_fig4_data$region, levels = c("Indo-Gangetic Plain", "National Average", "All other regions (excluding Indo-Gangetic Plain)"))
...
north_india_fs_fig4 <- ggplot(north_india_fs_fig4_data) +
  ...
  scale_y_continuous(breaks = seq(0, 100, 10), limits = c(0, 100)) +
  scale_x_continuous(breaks = c(seq(1998, 2019, 3), 2021)) +
    scale_color_manual(values = c("Indo-Gangetic Plain" = "darkred", "National Average" = "darkgrey", "All other regions (excluding Indo-Gangetic Plain)" = "orange")) +
  ggthemes::theme_tufte() +
  labs(x = "Year", 
       y = expression("Annual Average   " ~ PM[2.5] ~ "(in µg/m³)"), 
       title = expression("Annual Average" ~ PM[2.5] ~ "concentrations in the Indo-Gangetic Plain and other regions of India, 1998 to 2021")) +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 11),
        legend.title = element_blank(),
        axis.title.y = element_text(size = 13, margin = margin(r = 0.6, unit = "cm")), 
